...
try:
    # Suppress future cryptography deprecation warnings about serial number etc.
...
def condense_checks(checks):
    """Rút gọn 8 dòng kết quả về dạng: 'n) HỢP LỆ' / 'n) KHÔNG HỢP LỆ' / 'n) KHÔNG CẦN'."""
    out = []
    for line in checks:
        try:
            ...
            if m:
                num = m.group(1)
                status = m.group(2)
                # bỏ chú thích trong ngoặc nếu có
                status = status.split('(')[0].strip()
                out.append(f"{num}) {status}")
            else:
                out.append(line)
        except Exception:
            out.append(line)
...
def _parse_status(line: str) -> str:
    ...
    if m:
        ...
        status = status.split('(')[0].strip()
        return status
    # already condensed like 'n) HỢP LỆ'
...
def make_pretty_output(checks, pdfpath: str) -> str:
    """Render checks in a compact, chuyên nghiệp style with icons and alignment."""
    titles = [
        "ByteRange & Contents",
        "PKCS#7",
        "messageDigest",
    ...
    statuses = [_parse_status(c) for c in checks]
    icon = {
        'HỢP LỆ': '✓',
        'KHÔNG HỢP LỆ': '✗',
        'KHÔNG CẦN': '•',
    ...
    header = "KẾT QUẢ KIỂM TRA CHỮ KÝ SỐ"
    lines = []
    lines.append(header)
    lines.append('=' * len(header))
    lines.append(f"Tệp: {pdfpath}")
    lines.append(f"Thời gian: {now}")
    lines.append('-' * 48)
    name_width = max(len(t) for t in titles)
    for i, (t, s) in enumerate(zip(titles, statuses), start=1):
        sym = icon.get(s, '-')
        lines.append(f"{i:>2}. {t.ljust(name_width)}  {sym} {s}")
    # Kết luận đơn giản: tất cả mục là HỢP LỆ hoặc KHÔNG CẦN -> HỢP LỆ
    ok = all(s in ('HỢP LỆ', 'KHÔNG CẦN') for s in statuses)
    lines.append('-' * 48)
    if ok:
        lines.append('KẾT LUẬN: HỢP LỆ')
    elif all(s in ('HỢP LỆ', 'KHÔNG CẦN', 'LOCAL TRUST') for s in statuses):
        lines.append('KẾT LUẬN: HỢP LỆ (LOCAL TRUST – chứng chỉ tự ký)')
    else:
        lines.append('KẾT LUẬN: KHÔNG HỢP LỆ')
    return "\n".join(lines)
...
def main(pdfpath, trust_local_pfx=False, short=False, pretty=False):
    ...
    try:
        # Capture noisy stdout/stderr from endesive to avoid verbose lines in console
        _buf = _io.StringIO()
        with contextlib.redirect_stdout(_buf), contextlib.redirect_stderr(_buf):
            ev = endesive_verify(data)
        # Ignore captured text intentionally
        try:
            first = ev[0]
            sig_ok = bool(first[0])
    ...
    # If local PFX available, compare its public key to ANY cert inside PKCS#7 bundle
    local_match = False
    try:
        pfx_path = 'cert.pfx'
        if os.path.exists(pfx_path):
            ...
            try:
                ...
                if cert_local is not None:
                    ...
                    # Try match with any certificate from SignedData
                    pk_list = []
                    try:
                        certs_sd = sd.get('certificates')
                        if certs_sd:
                            for c in certs_sd:
                                try:
                                    der_c = c.chosen.dump()
                                    cx = crypto_x509.load_der_x509_certificate(der_c)
                                    pk_list.append(cx.public_key())
                                except Exception:
                                    continue
                    except Exception:
                        pass
                    # If none collected, fallback to single 'cert' extracted earlier
                    if not pk_list and cert is not None:
                        pk_list = [cert.public_key()]
                    ...
                    for pk in pk_list:
                        try:
                            if hasattr(local_pub, 'public_numbers') and hasattr(pk, 'public_numbers'):
                                ...
                                if getattr(ln, 'n', None) == getattr(rn, 'n', object()) and getattr(ln, 'e', None) == getattr(rn, 'e', object()):
                                    lines.append('- Local PFX public key MATCHES a cert in PDF (modulus/exponent equal).')
                                    local_match = True
                                    break
                        except Exception:
                            continue
                    if not local_match:
                        lines.append('- Local PFX public key DOES NOT match any cert in PDF bundle.')
            except Exception as e:
    ...
    # certvalidator available -> attempt to validate chain and revocation
        ...
        if trust_local_pfx and os.path.exists('cert.pfx'):
            try:
                ...
                priv_local, cert_local, add_local = load_key_and_certificates(pfx_data, b'1234')
                # Ưu tiên dùng CA trong pfx (add_local) làm trust roots; nếu không có thì dùng cert_local
                roots = []
                try:
                    if add_local:
                        for c in add_local:
                            if c is not None:
                                roots.append(x509.Certificate.load(c.public_bytes(encoding=Encoding.DER)))
                except Exception:
                    pass
                if not roots and cert_local is not None:
                    roots.append(x509.Certificate.load(cert_local.public_bytes(encoding=Encoding.DER)))
                if roots:
                    trust_roots = roots
                    lines.append(f'- Thêm {len(roots)} chứng chỉ từ cert.pfx vào trust_roots (--trust-local-pfx).')
            except Exception as e:
    ...
    # Gắn nhãn LOCAL TRUST thay vì HỢP LỆ nếu chỉ dựa vào chứng chỉ tự ký cục bộ
    try:
        if checks[4].startswith('5) Chain -> trusted root CA: KHÔNG') and local_match:
            checks[4] = '5) Chain -> trusted root CA: LOCAL TRUST'
        if checks[5].startswith('6) OCSP/CRL check: KHÔNG') and local_match:
            checks[5] = '6) OCSP/CRL check: LOCAL TRUST'
    except Exception:
        pass
    ...
    # finalize: in pretty mode, in ra khung đầy đủ và GHI LOG ĐÚNG NHƯ ĐÃ HIỂN THỊ
    if pretty:
        checks_out = condense_checks(checks)  # để tính kết luận và tiêu đề gọn
        pretty_text = make_pretty_output(checks_out, pdfpath)
        # Ghi log chính xác phần hiển thị
        try:
        ...
        checks_out = condense_checks(checks) if short else checks
        write_checks_log(checks_out)
        print('\n'.join(checks_out))
    return 0
...
if __name__ == '__main__':
    ...
    if '--pretty' in args:
        pretty = True
        # pretty implicitly condenses content on file; keep flag to render frame
        # but allow both --short and --pretty together (harmless)
        args.remove('--pretty')
    pdfpath = args[0] if len(args) > 0 else DEFAULT_PDF
    sys.exit(main(pdfpath, trust_local_pfx=trust_local, short=short, pretty=pretty))
